...
class Flow {
    ...
    constructor(parent,globalFlow,flow) {
        ...
        // callback is an array of middleware definitions
        // _middleware is an object for direct lookup of middleware callback -> value
        this._middleware = []
    }
    ...
    async start(diff) {
        ...
        if (this.isGlobalFlow) {
            ...
            for (let i = 0; i < configNodes.length; i++) {
                ...
                if (node.type === 'global-config' && Array.isArray(node.middleware)) {
                    this._middleware = node.middleware.map(mw => mw.callback)
                }
            }
        }
